...
def get_x_hat_in_2d(x_o: torch.Tensor, x_adv: torch.Tensor, axis_unit1: torch.Tensor, axis_unit2: torch.Tensor,
                    net: torch.nn.Module, queries, original_label, max_iter=2,plus_learning_rate=0.01,minus_learning_rate=0.0005,half_range=0.1, init_alpha = np.pi/2, history = []):
    ...
    def get_alpha(A):
        """Get alpha from history"""
        alpha_values = [t[0] for t in A]
        return alpha_values

    # Define Q-learning parameters
    alpha_values = get_alpha(history)  # Possible alpha values from 0 to 90
    num_actions = len(alpha_values)
    num_states = 1  # Number of states (in this case, only alpha is considered as the state)
    ...
    def update_q_table(state, action, next_state, reward):
        ...
        q_table[state, action] = q_value + learning_rate * (reward + discount_factor * max_q_value - q_value)
    ...
    bo = BayesianOptimization(f=objective, pbounds=bounds, random_state=42, allow_duplicate_points=True)
    bo.maximize(init_points = 5, n_iter=0) 
    ...
    for i in range(max_iter): #max_iter is set to 2?
        ...
        if label != original_label:
            ...
            next_alpha = alpha_values[select_action(state)]  # Select action based on epsilon-greedy exploration
            score = objective(next_alpha)
            bo.register(params={'alpha': next_alpha}, target=score)
            reward = score - bo.max['target']  # Calculate reward as the difference between current score and best score
            next_state = 0  # Next state is always 0, as we only have one state (alpha)
            update_q_table(state, select_action(state), next_state, reward)  # Update Q-table based on Q-learning rule
