...
def get_model(args,device):
    ...
    if model_name == 'resnet-18':
        ...
        model = VisionTransformer(CONFIGS["ViT-L_16"], 512, zero_head=False, num_classes=1000)
        model.load_from(np.load(os.getcwd()+"/imagenet21k+imagenet2012_ViT-L_16.npz"))  
        model.eval()
        model.to(device)

        fmodel = PyTorchModel(model, bounds=(0, 1))
        return fmodel
    elif model_name == 'BiT-M':
        #model = BigTransferModels.KNOWN_MODELS["BiT-M-R101x1"](head_size=1000, zero_head=False)
        #model.load_from(np.load(os.getcwd()+"//BiT-M-R101x1-ILSVRC2012.npz"))
        model = BigTransferModels.KNOWN_MODELS["BiT-M-R152x4"](head_size=1000, zero_head=False)
        model.load_from(np.load(os.getcwd()+"//BiT-M-R152x4-ILSVRC2012.npz"))
        model.eval()
        model.to(device)
        ...
        model = VisionTransformer(CONFIGS["ViT-B_16"], 224, zero_head=False, num_classes=1000)
        model.load_from(np.load(os.getcwd()+"/imagenet21k+imagenet2012_ViT-B_16-224.npz"))  
        model.eval()
        model.to(device)
